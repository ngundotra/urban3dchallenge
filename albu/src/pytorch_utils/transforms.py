...
class ShiftScaleRotate:
    ...
    def __call__(self, img, mask=None):
        if random.random() < self.prob:
            ...
            # Create rotation matrix for np operations & scale
            ...
            # Create the rotated & shifted array
            ...
            img = cv2.warpPerspective(img, mat, (width, height),
                                      ...
                                      borderMode=cv2.BORDER_REFLECT_101)
            if mask is not None:
                # Use the same transformation on the label (mask)
...
class RandomHueSaturationValue:
    """Randomly shifts the HSV of an image by uniformly random amounts in a given limit"""
...
class ToTensor:
    ...
    def __call__(self, im, mask=None):
        ...
        if mask is not None:
            mask = np.expand_dims(mask / (255. if mask.dtype == np.uint8 else 1), axis=0).astype(np.float32)
    ...
    return DualCompose([
        OneOrOther(
            *(OneOf([
              ...
              ShiftScaleRotate(shift_limit=0.0625, scale_limit=0.10, rotate_limit=20)), prob=prob),
        RandomFlip(prob=0.5),
        Transpose(prob=0.5),
...
def augment_flips_color(x, mask=None, prob=.5):
    return DualCompose([
        RandomFlip(prob=.5),
        Transpose(prob=.5),
